...
const packNames = WORD_COLLECTIONS;
...
function loadState() {
  try {
    ...
    saveUpgradeNeeded = data.version < 3 || data.settings?.packVersion !== 1;
...
async function importFile(file) {
  ...
  try {
    ...
    const apply = () => {
      ...
      if (!forPack(words, state.settings.pack).length) state.settings.pack = 'all';
...
document.addEventListener('change', event => {
  ...
  if (['pack', 'word-pack'].includes(el.dataset.change)) { state.settings.pack = el.value; state.settings.packVersion = 1; wordPage = 0; persist(); render(); }
  if (el.dataset.change === 'sort') { wordSort = el.value; wordPage = 0; renderWordTable(); }
});
document.addEventListener('submit', event => {
  if (event.target.id !== 'settings-form') return;
  event.preventDefault(); const data = new FormData(event.target);
  state.settings = normalizeSettings({ ...Object.fromEntries(data), packVersion: 1, goalVersion: 2, effects: data.has('effects'), sound: data.has('sound'), bonus: data.has('bonus') });
...
async function init() {
  ...
  const results = await Promise.allSettled(['dictionary.json', 'StrangerThings.json'].map(async name => {
    const response = await fetch(new URL(`./data/${name}?v=2.5.0`, import.meta.url));
  ...
  if (!forPack(words, state.settings.pack).length) { state.settings.pack = 'all'; saveUpgradeNeeded = true; }
