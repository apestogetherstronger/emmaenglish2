...
import { PICTURE_WORDS, pictureChoices } from '../dist/picture-core.js';
...
assert.equal(PICTURE_WORDS.length, 48);
assert.equal(new Set(PICTURE_WORDS.map(w => w.id)).size, 48);
for (const category of ['animals', 'food', 'objects']) {
...
console.log(`Validated both static pages, JavaScript, 48 picture words and assets, and 4/7-choice support across ${words.length} bilingual words and all ${Object.keys(WORD_COLLECTIONS).length} word collections.`);
