...
The **Song** filter combines **Aeroplane** and **Whenever, Wherever** into **119 unique entries** (53 + 74, with 8 shared). Every entry in either song carries the `song` tag as well as its individual song tag. All three filters are available on the learning page, in My words, and in practice settings, in both interface languages. All words remains the default, so song words also appear in regular mixed practice. Tags refer to the same vocabulary IDs and answer history; changing collections never duplicates a word or awards XP. Picture Mode's vocabulary and separate progress are unaffected.
